...
Tracks read/write/fetch access counts for every address in the 64K address
space.  Produces summary reports in various formats after tracing completes.
"""
...
class MemoryTracer:
    ...
    def __init__(self):
        ...
        self.fetches = bytearray(65536 * 4)
    ...
    def record_fetch(self, addr):
        self._inc(self.fetches, addr)
    ...
    def get_fetch_count(self, addr):
        off = addr * 4
        return int.from_bytes(self.fetches[off:off+4], 'little')
    ...
    def _report_text(self, file):
        ...
        x = code fetched, R = data read, W = data write,
        X = data read+write, - = no access.
        """
        bytes_per_cell = 64
        cols = 64
        rows = 16

        print("\nMemory access map (x=code R=read W=write X=read+write "
              "-=none, each char=64 bytes):", file=file)
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                for offset in range(bytes_per_cell):
                    ...
                    if self.get_fetch_count(addr) > 0:
                        has_fetch = True
                        break  # code cell trumps everything
